...
assert.match(swift, /private enum RootModalRoute: Identifiable/);
assert.match(swift, /\.fullScreenCover\(item: \$modalRoute, onDismiss: presentPendingSharedQuoteIfPossible\)/);
assert.match(swift, /TodayView \{\s*modalRoute = \.settings\s*\}/);
...
assert.doesNotMatch(swift, /@State private var showingSettings/);
assert.doesNotMatch(swift, /\.sheet\(isPresented: \$showingSettings\)/);
assert.match(swift, /TodayView \{[\s\S]{0,100}modalRoute = \.settings[\s\S]{0,160}\.tabItem/);
