# Metadatos de App Store — iOS 1.0 (Español)

Estado: localización Española (España) guardada en la ficha privada de App Store Connect el 2026-08-10. Faltan capturas, URL de privacidad, contacto de revisión, copyright y build. No subir ni publicar sin autorización expresa.

## Página de producto

- **Nombre en App Store:** `Warm Words: Frases Diarias`
- **Nombre en el dispositivo:** `Warm Words`
- **Subtítulo:** `Una frase para cada día`
- **Idioma principal de la app:** Inglés (EE. UU.)
- **Localización:** Español (España)
- **Categoría principal:** Estilo de vida
- **Categoría secundaria:** Salud y forma física
- **Precio:** Gratis
- **Disponibilidad:** 175 países o regiones
- **Clasificación por edades:** 9+ en 172 países o regiones; 12+ en Vietnam y Brasil; All en Corea del Sur
- **Dispositivos:** iPhone; disponibilidad automática en Mac y Apple Vision Pro desactivada
- **Versión:** 1.0
- **Apple ID:** `6800058458`
- **Bundle ID:** `com.dmkr.inspiraciondia`
- **SKU:** `com.dmkr.inspiraciondia`
- **URL de soporte:** `https://github.com/Krazel/InspiracionDia/issues` (provisional)
- **URL de privacidad:** `https://github.com/Krazel/InspiracionDia/blob/agent/warm-words-ios-ipa/docs/release/PRIVACY_POLICY_ES.md`

### Texto promocional

Reserva un momento para una frase que te acompañe cada día, guarda las palabras que conecten contigo y programa un recordatorio suave.

### Descripción

Empieza el día con una frase inspiradora elegida de una colección tranquila y cuidada.

Usa Warm Words en inglés o en español y cambia de idioma cuando quieras desde Ajustes.

Explora 180 frases originales sobre motivación, enfoque, calma, disciplina, autoestima, gratitud, valentía, hábitos, creatividad, resiliencia, relaciones y energía.

Con Warm Words puedes:

- Ver una frase diferente cada día.
- Explorar frases por categoría.
- Guardar tus favoritas en el dispositivo.
- Compartir frases como bonitas tarjetas visuales.
- Añadir y eliminar tus propias frases personales.
- Programar un recordatorio local para la hora y los días que elijas.
- Elegir qué categorías pueden aparecer en los recordatorios.
- Cambiar entre los catálogos completos en inglés y español sin perder favoritos ni preferencias.

No necesitas una cuenta. Warm Words funciona sin conectarse a ningún servicio. Tus preferencias y frases personales permanecen en tu dispositivo.

### Palabras clave

`frases,inspiración,motivación,positividad,gratitud,enfoque,recordatorio,bienestar`

## Pendiente antes de revisión

- Capturas reales en español después del QA físico.
- Sustituir las URL provisionales de GitHub por páginas públicas propias antes del lanzamiento si están disponibles.
- Copyright y contacto de revisión.
- Copyright, contacto de revisión y fecha final de lanzamiento.
- Archive firmado y build validada.
